...
# Set from --json in main(); when True, commands emit JSON to stdout and all
# decorative/progress output to stdout is suppressed to keep the stream clean.
JSON_OUTPUT = False
...
class Spinner:
    ...
    def start(self):
        # Stay silent in JSON mode so the spinner never writes to stdout.
        if JSON_OUTPUT:
            return
        self.running = True
...
LOGO = r"""
    ...
    ─────────────────────────────────────────────────────
      ...
      ██╔════╝ ██║      ██║    ─────────────────────────────────────
      ██║      ██║      ██║    search, basket, history - all from the cli
      ██║      ██║      ██║
       ██████╗ ███████╗ ██║    v{version}
       ╚═════╝ ╚══════╝ ╚═╝
"""
...
def emit(data, human: str) -> None:
    """
    Emit a command result.

    With --json, serialize ``data`` as JSON to stdout. Otherwise print the
    pre-formatted human-readable ``human`` text (existing behavior).
    """
    if JSON_OUTPUT:
...
def emit_error(message: str, command) -> int:
    """
    Emit an error and return exit code 1.

    With --json, print a ``{"error": ..., "command": ...}`` object to stdout so
    an agent can parse a single stream and check the exit code. Otherwise print
    the message to stderr (existing behavior). Returns 1 for convenience.
    """
    if JSON_OUTPUT:
        print(json.dumps({"error": message, "command": command},
...
def cmd_search(auth: AuthTokens, args: argparse.Namespace) -> int:
    ...
    if not products:
        if JSON_OUTPUT:
            return emit_error(f"No products found for '{query}'", "search")
...
def cmd_basket(auth: AuthTokens, args: argparse.Namespace) -> int:
    ...
    if not lines:
        emit(basket, "Your basket is empty.")
        return 0

    total = sum(line.get("Price", 0) for line in lines)
    human = (
        f"\nBasket ({len(lines)} items):\n\n"
        + "\n".join(format_basket_line(line) for line in lines)
        + f"\n\n  Total: {total:.2f} kr"
    )
    emit(basket, human)
    return 0
...
def cmd_add(auth: AuthTokens, args: argparse.Namespace) -> int:
    ...
    added_line = next((line for line in lines if line.get("Id") == product_id), None)

    if added_line:
        head = "\nAdded to basket:\n" + format_basket_line(added_line)
    else:
        head = f"Product {product_id} added to basket."
...
def cmd_details(auth: AuthTokens, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if JSON_OUTPUT:
            return emit_error(str(e), "details")
...
def cmd_history(auth: AuthTokens, args: argparse.Namespace) -> int:
    ...
    if order_id:
        ...
        if not order:
            msg = f"Order {order_id} not found in last {MAX_ORDER_HISTORY_LOOKUP} orders."
            if JSON_OUTPUT:
                return emit_error(msg, "history")
...
def main():
    ...
    # Parent parser so --json works both before and after the subcommand.
    # default=argparse.SUPPRESS avoids the argparse pitfall where a subparser's
    # default would overwrite a --json given at the top level.
...
Examples:
  ...
  %(prog)s history
    ...
    # Search command
    search_parser = subparsers.add_parser("search", parents=[global_parser], help="Search for products")
    search_parser.add_argument("query", help="Search query")
    search_parser.add_argument("-l", "--limit", type=int, default=10, help="Max results (default: 10)")

    # Details command
    details_parser = subparsers.add_parser("details", parents=[global_parser], help="Show detailed product info")
    details_parser.add_argument("product_id", help="Product ID to view")

    # Basket command
    subparsers.add_parser("basket", parents=[global_parser], help="Show current basket")

    # Add command
    add_parser = subparsers.add_parser("add", parents=[global_parser], help="Add product to basket")
    ...
    # History command
    history_parser = subparsers.add_parser("history", parents=[global_parser], help="Show order history")
    ...
    # No command: show help (or a JSON error in JSON mode).
    if args.command is None:
        if JSON_OUTPUT:
            return emit_error("No command given", None)
        parser.print_help()
        return 1
    ...
    try:
        ...
        if JSON_OUTPUT:
            return emit_error(str(e), args.command)
        print(f"Error: {e}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        # Single command mode - show welcome banner (suppressed in JSON mode)
        if not JSON_OUTPUT:
        ...
        if args.command == "search":
            ...
            if JSON_OUTPUT:
                return emit_error(f"Unknown command: {args.command}", args.command)
            print(f"Unknown command: {args.command}", file=sys.stderr)
            return 1

    except requests.exceptions.HTTPError as e:
        if JSON_OUTPUT:
            return emit_error(f"HTTP Error: {e}", args.command)
